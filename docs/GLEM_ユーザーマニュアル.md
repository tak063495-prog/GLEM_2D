...
| 1.2.0 | 2026-08-25 | インストーラー、配布物検証、キーボード操作、アクセシビリティ、Janbu近似の注意事項を追加 | GLEM 開発チーム |
...
GLEM は.NETランタイム同梱の自己完結型アプリとして、インストーラー版とポータブルZIP版を配布します。

1. GitHub Releaseから `GLEM-<version>-win-x64-Setup.exe`、SHA-256ファイル、SBOMをダウンロードします。
2. SHA-256を確認してからインストーラーを実行します。インストーラーはスタートメニューへ登録し、`.glem` をGLEMへ関連付けます。
3. インストールせず使う場合は `GLEM-<version>-win-x64.zip` を任意のフォルダへ展開し、`GLEM.exe` を起動します。
...
GLEM のプロジェクトファイルは JSON 形式の `.glem` です。インストーラー版では自動的に関連付けられます。ポータブル版でWindowsのダブルクリック起動を有効にする場合は:
...
インストーラー版はWindowsの「インストールされているアプリ」からアンインストールします。ポータブル版は展開したフォルダを削除します。次のデータは必要に応じて手動で削除してください:
...
| 保存 | File → Save | `Ctrl+S`。最初に実行した場合は「名前をつけて保存」になります |
| 名前をつけて保存 | File → Save As... | `Ctrl+Shift+S`。任意のパスに `.glem` として保存します |

新規作成は `Ctrl+N`、開くは `Ctrl+O` でも実行できます。主要な入力欄と操作ボタンには論理的なタブ移動順とスクリーンリーダー向け名称を設定しています。Windowsのハイコントラスト表示にも追従します。
...
| ヤンブ型GLEM近似 | 非円（制御点を結ぶ折れ線） | 層状地盤や不規則な滑動面の概略比較。完全なJanbu一般化法ではないため、画面の警告と `docs/METHODS.ja.md` の制限を確認する |
...
| 地下水位線 | **破線**（§4.3） |
| 臨界滑動面 | **太い密破線**と凡例で識別する（色だけに依存しない）。円弧の場合は中心座標と半径が注記され、Janbu 制御点の場合は折れ線とマーカーで表示される |
...
- **限界平衡法**：滑動体を条体に分割し、各条体の力学的平衡から抵抗力と駆動力の比（安全率 FS）を求める方法。FS が大きいほど安定である
  ...
  - *GLEMのJanbu型近似*: 任意形状（非円）の滑動面へ適用できるが、完全なJanbu一般化法の実装ではない。円弧では λc = 1.0、非円では曲率偏差から求めるGLEM独自のヒューリスティック補正を適用する。画面とレポートの警告、および `docs/METHODS.ja.md` の式・適用限界を確認すること
