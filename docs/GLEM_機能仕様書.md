...
flowchart TD
    ...
    APP --> SA[斜面安定解析エンジン<br>Fellenius / Bishop簡化法 / ヤンブ型GLEM近似]
...
| M-3 | ヤンブ型GLEM近似 | 非円（制御点を結ぶ折れ線） | Fellenius型の力の釣合いにGLEM固有の補正係数 λc を適用 | 層状地盤・不規則滑動面の概略比較 |
...
**M-3 ヤンブ型GLEM近似（非円滑動面）**

滑動面を制御点間の折れ線で定義し、各線分の傾き α_i を使用する。抵抗力・駆動力はFellenius型に集計し、円弧では λc=1.0、非円では底面角の加重平均偏差から求めるGLEM固有のヒューリスティック補正を適用する。完全なJanbu一般化法のスライス間力関数を解く実装ではない。

$$
FS \;=\; \frac{\sum\left(c'\,\Delta L_i + N'_i\,\tan\phi'\right)}{\lambda_c \cdot \sum W_i\,\sin\alpha_i}
$$

`d_i=max(0,W_i sin α_i)`、`ᾱ=Σ(d_i α_i)/Σd_i` とし、`λc=min(2.0, 1+Σ(d_i|α_i-ᾱ|)/Σd_i)` とする。詳細な仮定・適用範囲・制限は `docs/METHODS.ja.md` を正とする。
...
| T-03 | ヤンブ型GLEM近似の回帰 | 非円滑動面の固定ケース | 実装式による独立計算値と一致 | §4.7 A-1 |
