...
| F-03 | 斜面安定解析 | Fellenius法・Bishop簡化法・Janbu一般化条体法による FS 算出と臨界滑動面探索 |
...
| ID | リスク・制約 | 影響 | 対策 |
|---|---|---|---|
| RISK-01 | Janbu法の補正係数 λc の算出精度（詳細設計書 D-04） | FS の精度に直結する | 公開例（T-03）でテーブル値を検証し、誤差が A-1（±0.01）内であることをリリース前に確認する |
